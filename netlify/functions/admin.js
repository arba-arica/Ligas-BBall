  ...
  try {
    ...
    switch (action) {
      ...
      case 'crearPartido': {
        requireOrg(session);
        if (session.rol !== 'admin') await verifyCategoriaTorneo(data.categoria_id, session.org_id);
        // Validar: local ≠ visita (solo si ambos vienen definidos; null = "por definir", ej. semifinales/finales pendientes)
        if (data.equipo_local && data.equipo_visita && data.equipo_local === data.equipo_visita) {
        ...
        const { data: partido, error } = await db.from('partidos').insert({
          ...
          equipo_visita:   data.equipo_visita || null,
